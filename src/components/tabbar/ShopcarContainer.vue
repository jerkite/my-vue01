    ...
    <div class="mui-card">
      <div class="mui-card-content">
        <div class="p-content">
          ...
          <p>
            已勾选商品
            <span class="red">{{$store.getters.getAllCountAndAmount.count}}</span>，总计
            <span class="red">￥{{$store.getters.getAllCountAndAmount.amount}}</span>
          </p>
        </div>
...
export default {
  ...
  methods: {
    getcarinfo() {
      ...
      this.$http
        .get("api/goods/getshopcarlist/" + carid.join(","))
        .then(result => {
          //join方法可将所有数组元素结合为一个字符串。获取多条数据
    ...
    remove(id, index) {
      //store根据id来删除数据，shopcarlist根据索引index删除数据
      this.shopcarlist.splice(index, 1);
      this.$store.commit("removeFromCar", id);
    },
    SelectChange(id, val) {
      //向store传id跟isselect的布尔值
